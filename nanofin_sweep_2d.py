...
def run_2d_sweep(config_path, output_base_dir="nanofin_2d_sweep_g"):
    print("=" * 70)
    print("NANOFIN 2D SWEEP (WAVELENGTH × ANGLE)")
    print("=" * 70)
    ...
    wavelengths = np.arange(405, 705, 5)
    angles = np.arange(0, 180, 45)
    
    total_wavelengths = len(wavelengths)
    total_angles = len(angles)
    total_simulations = total_wavelengths * total_angles
    
    print(f"Simulation shape: {sim_shape}")
    print(f"Grid size: {dL} nm/pixel")
    print(f"Wavelength range: {wavelengths[0]} - {wavelengths[-1]} nm ({total_wavelengths} wavelengths)")
    print(f"Angle range: {angles[0]} - {angles[-1]} degrees ({total_angles} angles)")
    ...
    for wl_idx, wavelength in enumerate(wavelengths):
        print(f"Wavelength {wl_idx+1}/{total_wavelengths}: {wavelength} nm")
        
        wvln_dir = output_dir / f"wavelength_{wavelength:03d}nm"
        wvln_dir.mkdir(exist_ok=True)
        
        config_wavelength = copy.deepcopy(config)
        config_wavelength['wavelength'] = float(wavelength)
        
        wvln_results = []
        
        for angle_idx, angle in enumerate(angles):
            simulation_count += 1
            print(f"  Angle {angle_idx+1}/{total_angles}: {angle}° (Sim {simulation_count}/{total_simulations})")
            
            kwargs_angle = copy.deepcopy(kwargs)
            kwargs_angle['fin_angle_deg'] = float(angle)
            
            angle_dir = wvln_dir / f"angle_{angle:03d}deg"
            ...
            try:
                ...
                wvln_results.append(result_entry)
        ...
        print(f"  Completed wavelength {wavelength}nm ({len(wvln_results)} simulations)")
        print()
    
    total_time = time.time() - start_time_total
    
    print("=" * 70)
    print("2D SWEEP COMPLETED")
    print("=" * 70)
    print(f"Total time: {total_time:.2f}s")
    print(f"Average time per simulation: {total_time/total_simulations:.2f}s")
    print(f"Results saved to: {output_dir}")
    
    successful = [r for r in results_summary if 'error' not in r]
    failed = [r for r in results_summary if 'error' in r]
    
    print(f"Successful runs: {len(successful)}/{total_simulations}")
    if failed:
        print(f"Failed runs: {len(failed)}")
        for fail in failed[:5]:
            print(f"  - Wavelength {fail['wavelength_nm']}nm, Angle {fail['angle_deg']}°: {fail['error']}")
        if len(failed) > 5:
...
def main():
    config_path = "nano_atom/configs/nanofin_b.yaml"
    output_path = "nano_atom/results/nanofin_2d_sweep_b"
    
    if not Path(config_path).exists():
        print(f"Error: Configuration file not found: {config_path}")
        return
    
    results = run_2d_sweep(config_path, output_path)

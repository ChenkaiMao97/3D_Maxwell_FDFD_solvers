...
class PlaneWaveSource:
    ...
    def __call__(
        ...
        print("source store_dir: ", self._params.store_dir)
        source_exist = os.path.isfile(self._params.store_dir+'inputs/cached_source.npy')
        kvector_exist = os.path.isfile(self._params.store_dir+'inputs/cached_kvector.npy')
        if source_exist and kvector_exist:
            source=np.load(self._params.store_dir+'inputs/cached_source.npy')
            kvector=np.load(self._params.store_dir+'inputs/cached_kvector.npy')
            ...
            # os.mkdir(store_dir+'inputs')
            np.save(self._params.store_dir+'inputs/cached_source', source)
            np.save(self._params.store_dir+'inputs/cached_kvector', kvector)
...
class FdfdSimulation(problem.OptimizationFunction):
    ...
    def eval(self,
             ...
             save_folder: Optional[str] = None) -> np.ndarray:
        """Runs the simulation.
        ...
        Returns:
            Simulated fields.
        """
        return self._simulate(input_val[0],save_folder)
    ...
    def _simulate(self,
        ...
        if electric_fields is None:
            # Perfrom the solve.
            electric_fields, solve_time, error = self._solver.solve(
                omega=2 * np.pi / self._wlen,
                ...
                store_dir = save_folder,
                solver_info=True
            )
            print(f"solve_time: {solve_time}")
            print(f"iterations finished: {len(error)}, final error: {error[-1]}")
            with open(save_folder+'solver_info.txt', 'w') as f:
                f.write(f"solve time: {solve_time}\n")
                f.write(f"{error}")
...
class ImportOverlap:
    ...
    def __call__(self, simspace: SimulationSpace, wlen: float = None,
        ...
        overlap_grid = np.zeros(reference_grid.grids.shape, dtype=np.complex128)

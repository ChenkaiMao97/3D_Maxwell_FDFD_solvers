...
from src.invde.utils.jax_utils import box_downsample, kronecker_upsample, extract_fixed_solid_void_pixels, ResponseArray, Density, grayscale_initializer
...
class BaseChallenge:
    ...
    def __init__(
        ...
        density_initializer = grayscale_initializer,
        _backend: str = 'NN',
...
def _extract_fixed_solid_void(
    ...
    model = problem_constructor(dL=dL, wavelengths=wavelengths, pmls=pmls, _backend='NN')
    model.init(shape_only=True)

    density_with_gray_design = model.density(
        onp.ones(model.design_variable_shape[0:2]) * 0.5
    )

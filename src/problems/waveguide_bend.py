...
def plot_source_profile(source, source_slice):
    src = source[source_slice[0][0]:source_slice[1][0]+1, source_slice[0][1]:source_slice[1][1]+1, source_slice[0][2]:source_slice[1][2]+1].squeeze()
    plt.figure(figsize=(18, 12))
    plt.subplot(2,3,1)
    plt.imshow(src[:,:,0].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_xr')
    plt.colorbar()
    plt.subplot(2,3,2)
    plt.imshow(src[:,:,1].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_yr')
    plt.colorbar()
    plt.subplot(2,3,3)
    plt.imshow(src[:,:,2].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_zr')
    plt.colorbar()
    plt.subplot(2,3,4)
    plt.imshow(src[:,:,3].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_xi')
    plt.colorbar()
    plt.subplot(2,3,5)
    plt.imshow(src[:,:,4].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_yi')
    plt.colorbar()
    plt.subplot(2,3,6)
    plt.imshow(src[:,:,5].detach().cpu().numpy(), cmap='viridis')
    plt.title('source_zi')
    plt.colorbar()
    plt.tight_layout()
    plt.savefig('source.png', dpi=200)
    plt.close()
...
def make_waveguide_bend(sim_shape, wl, dL, pmls, kwargs):
    ...
    if kwargs['plot_source']:
        plot_source_profile(source, source_slice)

...
def solver_worker(device_id, init_kwargs, task_queue, result_queue):
    ...
    while True:
        ...
        if len(payload) == 7:
            epsilon_r_torch, source_torch, wl_torch, dl_torch, pml_thickness, gt, init_x = payload
            transpose_solve = False
        else:
            epsilon_r_torch, source_torch, wl_torch, dl_torch, pml_thickness, gt, init_x, transpose_solve = payload
        ...
        output = solver.solve(epsilon_r_torch, source_torch, gt=gt, init_x=init_x, transpose=transpose_solve)
